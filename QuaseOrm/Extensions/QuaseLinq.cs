...
namespace QuaseOrm.Extensions
{
    public static class QuaseLinq
    {
        ...
        **/
        ...
        public static T Include<T, TKey>(this T source, Func<T, TKey> Entidade) where T : new() where TKey : new()
        {
            ...
            if (valorChavePrimaria != null)
            {
                foreach (var i in properties)
                {
                    ...
                    if (a == b)
                    {
                        using (var db = new Session(connectionString))
                        {
                            obj1 = db.GetByCriteria<TKey>(new Criteria(chavePrimaria.Name, valorChavePrimaria));
        ...
        public static int Count<T>(this T source, Parameters parametros) where T : class
        {
            int contador;
            try
            {
                using (Session db = new Session(connectionString))
                {
                    contador = db.MCount<T>(parametros);
                }
                return contador;
            }
            catch (Exception ex)
